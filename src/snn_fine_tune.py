...
from src.utils import init_distributed, WarmupCosineSchedule
from src.losses import init_suncet_loss, make_labels_matrix
...
def main(args):
    ...
    imgs_per_class = (
        int(1300 * (1.0 - unlabeled_frac))
        if "imagenet" in dataset_name
        else int(5000 * (1.0 - unlabeled_frac))
    )
...
def load_pretrained(r_path, encoder, device, ckp=False):
    checkpoint = torch.load(r_path, map_location=device)
    if ckp:
        pretrained_dict = {k: v for k, v in checkpoint["encoder"].items()}
    else:
        pretrained_dict = {
            k.replace("module.", ""): v for k, v in checkpoint["encoder"].items()
        }
...
def load_from_path(
    r_path, encoder, opt, sched, scaler, device, use_fp16=False, ckp=False
):
...
def init_model(
    ...
    encoder.to(device)
    encoder = load_pretrained(  # Here load the previous pre-trained model
        r_path=r_enc_path, encoder=encoder, device=device
    )
